...
def yum_install(packages):
    ...
    if status == 0:
        ...
        return
    LOG.info('download packages %s', packages)
    download_dir = os.path.join('rpm_packages', '_'.join(packages))
    ...
    LOG.info('install packages %s', packages)
